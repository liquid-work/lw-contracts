...
contract SuperLiquidWork is SuperAppBase {
    ...
    // Superfluid 
    ISuperfluid private host; // Superfluid host <-- make a call to this.  
    ...
    // mappings 
    mapping (address => uint256) public senderServiceId; // mapping tracking user recent service to serviceId 
    ...
    //event ServiceApproved(uint256 _serviceId);
    ...
    /*modifier onlyJudge(uint256 _serviceId) {
        //console.log(_serviceId);
        require(services[_serviceId].judge == msg.sender, "Only judge Allowed");
        _;
    }
    */
    ...
    modifier noServiceStarted() {
        ...
        uint256 serviceId = senderServiceId[msg.sender];
        if(serviceId > 0) {
            require(services[serviceId].status != ServiceStatus.NOT_STARTED, 'You already started a service');
        }
    ...
    /// @dev function for user to create a custom service
    function createService(string memory _name, uint256 _serviceId, address _sender, address _receiver, ServiceStatus _status, int96 _flowRate,  uint256 _expirationDate, uint256 _totalAmountStreamed) external noServiceStarted {
        ...
        senderServiceId[msg.sender] = servicesCount;
        emit ServiceCreated(
            servicesCount,
            msg.sender,
            _receiver,
        ...
        returns (bytes memory newCtx)
    {
        ...
        uint256 serviceId = senderServiceId[sender];
